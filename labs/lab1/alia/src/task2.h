...
int init(double mt, double angle, bool output = false) {
    ...
    double r12x0 = , r12y0 = 141005424242.41,
    ...
    double t = 0.0;
    double t_circle_end = 60. * 60 * 24 * 28 * 3;
    ...
    int status = 0; //0 - нет столкновения в пределах орбиты спутника,1 - cтолкновение со спутником, 2 - столкновение с планетой
                    // 3 - ракета улетает далеко
    ...
    while (t < t_end) {
        ...
        // Проверка на то что ракета улетает от планеты далеко
        double dx_dist_rocket = y[0] - r12x0;
        double dy_dist_rocket = y[1] - r12y0;
        double dist_rocket = std::sqrt(dx_dist_rocket * dx_dist_rocket);
        if (dist_rocket > (Constants::R2 + Constants::R23 + Constants::R3)*5) {
            status = 3;
            break;
        }
